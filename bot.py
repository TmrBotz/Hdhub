...
def get_extraflix_post_urls():
    try:
        ...
        resp = requests.get(EXTRAFLIX_HOMEPAGE, headers=headers, timeout=20)
        resp.raise_for_status()

        soup = BeautifulSoup(resp.text, "html.parser")
        articles = soup.select("article.entry-card h2.entry-title a")

        urls = []
        for a in articles[:EXTRAFLIX_TOP_N]:
            href = a.get("href", "").strip()
            if href:
                urls.append(href)

        print(f"[ExtraFlix Scraper] Found {len(urls)} post URLs")
        return urls

    except Exception as e:
        print(f"[ExtraFlix Scraper] Homepage scrape failed: {e}")
        return []
...
def scrape_extraflix_post(post_url):
    try:
        ...
        if not data.get("success"):
            print(f"[ExtraFlix API] success=false for {post_url}")
            return None
...
def build_extraflix_message(data):
    ...
    if links:
        ...
        for item in links:
            ...
            if " - " in file_info:
                label = file_info.split(" - ")[0].strip()
            else:
                label = file_info.strip() or f"File ({file_size})"

            size_tag = f"[{file_size}]" if file_size else ""

            mirror_parts = []
            for i, url in enumerate(mirrors, 1):
                mirror_parts.append(f'<a href="{url}">Mirror {i}</a>')

            mirrors_str = " | ".join(mirror_parts)
            lines.append(f"• <b>{html.escape(label)}</b> {size_tag}")
...
def is_extraflix_sent(post_url):
    return col_extraflix.find_one({"url": post_url}) is not None


def mark_extraflix_sent(post_url, title):
    col_extraflix.update_one(
        {"url": post_url},
        {"$set": {"url": post_url, "title": title}},
        upsert=True
    )
...
def run_extraflix_job():
    print("[ExtraFlix Job] Starting check...")

    post_urls = get_extraflix_post_urls()
    if not post_urls:
        print("[ExtraFlix Job] No URLs found, skipping.")
        return

    new_count = 0

    for url in reversed(post_urls):
        if is_extraflix_sent(url):
        ...
        if not data:
            print(f"[ExtraFlix Job] Scrape failed, skipping: {url}")
            mark_extraflix_sent(url, "unknown")
            continue

        message = build_extraflix_message(data)
        success = send_telegram(message, EXTRAFLIX_CHANNEL_ID)

        if success:
            mark_extraflix_sent(url, data.get("title", "unknown"))
            new_count += 1
            time.sleep(2)
